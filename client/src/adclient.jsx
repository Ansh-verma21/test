...
function Adclient({ slide, here }) {
  ...
  const fetchDistricts = (value) => {
    axios.post("http://localhost:3000/get-dist", { dis_name: value })
      .then((res) => {
  ...
  const fetchLocations = (value) => {
    axios.get("http://localhost:3000/location-select")
      .then((res) => {
  ...
  const fetchCities = (value) => {
    axios.post("http://localhost:3000/get-cities", { name: value })
      .then((res) => {
  ...
  function adcl(event) {
    ...
    if (chaname === 1 && changing === 1 && checkpw === 1 && formData.login !== "" &&
      ...
      axios.post("http://localhost:3000/add2", dataToSend)
        .then(() => {
          setdone(1);
          setchan(0);
          setname(0);
  ...
  useEffect(() => {
    if (called === 1) {
      ...
      axios.post("http://localhost:3000/get-name", { client_name: formData.name })
        .then((res) => {
